...
echo "$($PSQL "SELECT GREATEST(MAX(winner_goals), MAX(opponent_goals)) AS most_goals_scored FROM games")"

echo -e "\nNumber of games where the winning team scored more than two goals:"
echo "$($PSQL "SELECT COUNT(*) FROM games WHERE winner_goals > 2")"

echo -e "\nWinner of the 2018 tournament team name:"
echo "$($PSQL "SELECT name AS winner_2018 FROM teams RIGHT JOIN games ON teams.team_id = games.winner_id WHERE year=2018 AND round='Final'")"
